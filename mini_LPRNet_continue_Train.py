...
if __name__ == '__main__':
    continue_saving='test_lr_mini_saving_ckpt(1.1_3_2)/test_lr_mini_saving_ckpt(1.1_3-1)'#
    ''' batch=24*2
    mini=math.pow( 4, 0.5 )
    p=0.5
    z2 = math.pow( 2, 0.5 )
    ...
    lprnet_load=torch.load(continue_saving+'/lprnet_Iter_074500_model.ckpt')#
    ...
    STN_load=torch.load(continue_saving+'/stn_Iter_074500_model.ckpt')#
    STN.load_state_dict(STN_load['net_state_dict'])
    ...
    with open(train_logging_file, 'a') as f:
        f.write("Continue--the r is:"+str(r)+'pow is:'+str(p)+'\n')
    f.close()
    ...
    if lprnet_load['iters']!=STN_load['iters']:
    ...
    learning_rate = 0.001*math.pow(0.915,int(total_iters/3000))
    ...
    for epoch in range(args.epoch):
        ...
        for imgs, labels, lengths in dataloader['train']:   # img: torch.Size([2, 3, 24, 94])  # labels: torch.Size([14]) # lengths: [7, 7] (list)
            ...
            if( (total_iters % 3000 == 0) and (total_iters > 0)):
